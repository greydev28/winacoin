...
export function Tokenomics() {
  return (
    <section id="tokenomics" className="bg-[#0a0614] py-24 sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        <SectionTitle title="Tokenomics" />
        <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <div className="rounded-2xl border border-fuchsia-400/30 bg-white/[.025] p-5 sm:p-8">
            <div className="grid items-center gap-8 md:grid-cols-[220px_1fr]">
              ...
              <div className="space-y-5">
                {allocations.map(([title, value, icon], i) => <div key={title} className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-fuchsia-500" /><div><b className="text-sm">{title}</b><p className="mt-1 text-xs leading-5 text-zinc-400">{icon === 'flame' ? '🔥 ' : ''}{value}{icon === 'drop' ? ' 💧' : ''}</p></div></div>)}
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-fuchsia-400/30 bg-white/[.025] p-5 sm:p-8">
