    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'border-b border-fuchsia-400/15 bg-[#07030f]/85 backdrop-blur-xl' : 'bg-transparent'}`}>
      <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-10">
        ...
        <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary navigation">
          ...
          <a href={externalLinks.buy} className="rounded-lg bg-gradient-to-r from-violet-700 to-fuchsia-500 px-6 py-3 text-xs font-black shadow-[0_0_30px_rgba(168,60,255,.25)] transition hover:-translate-y-0.5">BUY WINA</a>
        </nav>
      ...
      {open && (
        <nav className="mx-5 mb-4 rounded-2xl border border-fuchsia-400/20 bg-[#0b0616]/95 p-4 shadow-2xl backdrop-blur-xl lg:hidden" aria-label="Mobile navigation">
          <div className="flex flex-col gap-2">
            ...
            <a href={externalLinks.buy} onClick={() => setOpen(false)} className="mt-2 rounded-xl bg-gradient-to-r from-violet-700 to-fuchsia-500 px-4 py-3 text-center text-sm font-black">BUY WINA</a>
          </div>
        </nav>
      )}
    </header>
